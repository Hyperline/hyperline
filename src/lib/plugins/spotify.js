...
function status(comp) {
  spotify.isRunning((err, isRunning) => {
    if (!isRunning) {
      comp.setState({state: 'Not running'})
      return
    }
    spotify.getState((err, st) => {
      spotify.getTrack((err, track) => {
        var s = st.state == 'playing' ? '▶' : '❚❚'
        comp.setState({ state: s + ' ' + track.artist + ' - ' + track.name})
      })
    })
  })
}
    ...
    render() {
      ...
      return (
        <PluginWrapper color={fillColor}>
          <PluginIcon fillColor={fillColor} /> {this.state.state}
        </PluginWrapper>
      )
    }
  }
